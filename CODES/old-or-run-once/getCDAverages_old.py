...
if todayfile not in os.listdir("./daily_climate"):
    url = "https://geo.weather.gc.ca/geomet/features/collections/climate-daily/items?time=2018-01-01%2000:00:00/" + str(today) + "%2000:00:00&PROVINCE_CODE=ON&sortby=PROVINCE_CODE,CLIMATE_IDENTIFIER,LOCAL_DATE&f=csv&limit=150000&startindex=0"
...
for uid in cduid_list:
    ...
    if sub_df.avg_temp.isnull().all() == True and sub_df.min_temp.isnull().all() == True and sub_df.max_temp.isnull().all() == True and sub_df.avg_precip.isnull().all() == True:
        all_null.append(uid)
not_null = [uid for uid in cduid_list if uid not in all_null]
print("\n" + str(len(all_null)) + " have no average climate variables.")

###############################################################################
# FOR CENSUS DIVISIONS WITH NO WEATHER STATIONS, GET AVERAGE OF 5 CLOSEST DIVISIONS
print("\nGetting average of 5 closest divisions for divisions with no average climate variable...")

# If there are divisions with no weather stations
if len(all_null) != 0:
    ...
    # Get centroid of all Ontario divisions with weather stations
    ws = list(i for i in range(0, len(records)) if int(records[i][0]) not in all_null and records[i][3] == "35")
    centroids = {}
    for i in ws:
    ...
    # Get polygons of all divisions with no weather stations
    no_ws = list(i for i in range(0, len(records)) if int(records[i][0]) in all_null)
    polygons = {}
    for i in no_ws:
       polygon = Polygon(divisions.shape(i).points)
       polygons[int(records[i][0])] = polygon
    
    # Find 5 closest census districts
    closest = closestDivisions(polygons, centroids, 5)
    
    # Get average of closest 5 census districts
    df_cd_ws = df_cd[df_cd.cduid.isin(not_null)]
    data = []
    for uid in closest.keys():
        sub_df = df_cd_ws[df_cd_ws.cduid.isin(closest[uid])]
        for single_date in pd.date_range(start=start, end=today):
            sub_df_date = sub_df[sub_df.date == single_date]
            avg_temp = getAvg(sub_df_date, 2)
            min_temp = getAvg(sub_df_date, 3)
            max_temp = getAvg(sub_df_date, 4)
            avg_precip = getAvg(sub_df_date, 5)
        data.append([uid, single_date, avg_temp, min_temp, max_temp, avg_precip])
    col_names = ["cduid", "date", "avg_temp", "min_temp", "max_temp", "avg_precip"]
    df_cd_nows = pd.DataFrame(data, columns = col_names)
    df_cd = df_cd_ws.append(df_cd_nows, ignore_index = True)
    print("\nDivisions averages of divisions without weather stations dataset complete.")

###############################################################################
# FILL IN VALUES IF WEATHER STATIONS JUST DON'T RECORD FOR SOME DAYS

# Create dictionary of all division codes and dates that need filling in
...
if start == date(2018, 1, 1):
    ...
    df_cd_master.append(df_cd, ignore_index = True)
    df_cd_master.drop_duplicates(keep = "last")
